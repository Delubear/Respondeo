...
{
    // CCEL hyperlink-index cruft like "[6]" that directly precedes a reference token (e.g. "[6]Q[2]", "[991]FP" or the grouped link-table form "[476](Q[51], A[1])").
    // These are leftover anchor numbers from the source's link table and carry no reading meaning, so they are removed.
    // The lookahead keeps meaningful "Q[2]"/"A[2]" brackets (which are followed by punctuation or whitespace, never a letter or "(") intact.
    ...
    // An optional trailing objection citation like ", OBJ[3]" or ", Reply to OBJ[1]" is captured too. Run after the hyperlink cruft has been stripped.
    ...
    // Captures the first article number, any additional article numbers, and an optional trailing objection citation like ", OBJ[3]" or ", Reply to OBJ[1]".
